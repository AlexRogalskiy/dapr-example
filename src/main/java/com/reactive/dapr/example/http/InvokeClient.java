package com.reactive.dapr.example.http;
    ...
    public static void main(String[] args) throws Exception {
        try (DaprClient client = (new DaprClientBuilder()).build()) {
            for (String message : args) {
                Mono<byte[]> invokeMethod = client.invokeMethod(SERVICE_APP_ID, "say", message, HttpExtension.POST, null,
                        byte[].class);
                byte[] response = invokeMethod.block();
